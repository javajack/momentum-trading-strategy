...
def run_test(data, universe, start, end, dynamic=True, min_days=5, vix_decline=0.15, label=""):
    ...
    engine = BacktestEngine(
        universe=universe, historical_data=data, config=bt_config, app_config=config
    )
    result = engine.run()
    ...
    return {
        ...
        - (
            next((r for n, r in (result.benchmark_returns or []) if n.startswith("Nifty 50")), None)
            or 0
        ),
        "cagr": result.cagr,
...
for thresh in [0.10, 0.15, 0.20, 0.25, 0.30]:
    ...
    print(
        f"{thresh:.0%:<12} {r['return']:>9.1%} {r['alpha']:>9.1%} {r['max_dd']:>9.1%} {vix:>10} {r['trades']:>8}"
    )
...
_nifty_ret = (
    next((r for n, r in (result.benchmark_returns or []) if n.startswith("Nifty 50")), None) or 0
)
print(
    f"\nVIX DISABLED: Return={result.total_return:.1%}, Alpha={result.total_return - _nifty_ret:.1%}, Rebalances={rebal}, Trades={result.total_trades}"
)
...
for min_d in [5, 7, 10, 14, 21]:
    ...
    print(
        f"{min_d:<10} {r['return']:>9.1%} {r['alpha']:>9.1%} {r['max_dd']:>9.1%} {r['sharpe']:>7.2f} {r['rebalances']:>8} {r['trades']:>8}"
    )

# Fixed baseline
r_fixed = run_test(data, universe, start, end, dynamic=False)
print(
    f"{'Fixed':<10} {r_fixed['return']:>9.1%} {r_fixed['alpha']:>9.1%} {r_fixed['max_dd']:>9.1%} {r_fixed['sharpe']:>7.2f} {r_fixed['rebalances']:>8} {r_fixed['trades']:>8}"
)
...
for name, s, e in phases:
    ...
    if winner == "Dynamic":
        ...
        fix_wins += 1
    print(
        f"{name:<18} {r_dyn['return']:>9.1%} {r_fix['return']:>9.1%} {diff:>+9.1%} {r_dyn['max_dd']:>9.1%} {r_fix['max_dd']:>9.1%} {winner:>10}"
    )
...
for name, s, e in periods:
    ...
    print(
        f"{name:<8} {r_opt.total_return:>9.1%} {r_fix['return']:>9.1%} {diff_ret:>+9.1%} {r_opt.max_drawdown:>9.1%} {r_fix['max_dd']:>9.1%} {diff_dd:>+9.1%}"
    )

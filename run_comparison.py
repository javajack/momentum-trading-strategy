...
def run_backtest(
    universe, historical_data, config, start_date, end_date, rebalance_days, dynamic=True
):
...
def display_result(label, result):
    ...
    for bm_name, bm_ret in result.benchmark_returns or []:
        if bm_ret is not None:
            print(f"  {bm_name + ':':20s}{bm_ret:>10.2%}")
...
def main():
    ...
    result_dyn = run_backtest(
        universe, historical_data, config, start_date, end_date, 21, dynamic=True
    )
    display_result("Dynamic Rebalance (Default)", result_dyn)

    # Run 21-day fixed rebalance
    print("\n>>> Running 21-day FIXED rebalance...")
    result_21 = run_backtest(
        universe, historical_data, config, start_date, end_date, 21, dynamic=False
    )
    display_result("21-Day Fixed Rebalance (Monthly)", result_21)

    # Run 10-day fixed rebalance
    print("\n>>> Running 10-day FIXED rebalance...")
    result_10 = run_backtest(
        universe, historical_data, config, start_date, end_date, 10, dynamic=False
    )
    ...
    metrics = [
        (
            "Total Return",
            result_dyn.total_return,
            result_21.total_return,
            result_10.total_return,
            True,
        ),
        ("CAGR", result_dyn.cagr, result_21.cagr, result_10.cagr, True),
        (
            "Sharpe Ratio",
            result_dyn.sharpe_ratio,
            result_21.sharpe_ratio,
            result_10.sharpe_ratio,
            False,
        ),
        (
            "Max Drawdown",
            result_dyn.max_drawdown,
            result_21.max_drawdown,
            result_10.max_drawdown,
            True,
        ),
        ("Win Rate", result_dyn.win_rate, result_21.win_rate, result_10.win_rate, True),
        (
            "Total Trades",
            result_dyn.total_trades,
            result_21.total_trades,
            result_10.total_trades,
            False,
        ),
        (
            "Final Value",
            result_dyn.final_value,
            result_21.final_value,
            result_10.final_value,
            False,
        ),
    ]
